...
{
    public class InsiderIdentityBlobStoreFunction
    {
        ...
        public InsiderIdentityBlobStoreFunction(IStoreFeed storeFeed
        ...
        [FunctionName("InsiderIdentityBlobStoreFunction")]
        public async Task Run([EventGridTrigger] EventGridEvent eventGridEvent)
        {
            _telemetryHandler.TrackTraceInfo("Received Insider Feed Message");
            ...
            _telemetryHandler.TrackTraceInfo("Finished Processing Insider Feed Message");
        }
        ...
        private async Task SaveFeed(EventGridEvent eventGridEvent)
        {
            try
            {
                _telemetryHandler.TrackTraceInfo("Saving Insider Feed Message");
                ...
                var containerName = _configuration.GetValue(AppSettingsKey.IdentityInsiderFeedsContainer);
                ...
                if (string.IsNullOrWhiteSpace(feed))
                {
                    _telemetryHandler.TrackException(
                        new ApplicationException(
                            $"Received invalid or empty Feed. EventGrid Id: {eventGridEvent.Id}"));
                    return;
                }
